...
- For a pass/fail gate or a structured extraction that must match a JSON Schema, call `opentype_verdict`. Verdict runs are not served yet and answer `503 no_route_available`; then ask the gate as one `noul` question with `opentype_decide` (pass at 0.8 or above), and split an extraction into `choice`/`score` questions. With no valid judgment from either, fail closed.
...
- decide, verdict and route spend credit. Batch questions about one input into one call, and pass a stable `idempotency_key` when retrying.
