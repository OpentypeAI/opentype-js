...
- Every error from a paid call carries the key it sent in `error.idempotencyKey`, generated or yours. Sending the same request with that key replays the stored result instead of paying again.
- `runs.waitFor` throws `TimeoutError` with code `wait_timeout` when the run is not finished within `timeout`; each poll is a single attempt bounded by the time left, and an abort ends the wait at once.
